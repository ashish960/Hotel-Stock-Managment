    ...
    protected function configureRateLimiting(): void
    {
        ...
        RateLimiter::for('login', function (Request $request) {
         ...
         RateLimiter::for('user', function (Request $request) {
            return [
                Limit::perMinute(5000)->response(function (Request $request, array $headers) {
                    return response()->json([
                        'message' => 'Too many requests. Please try again later.',
                    ], Response::HTTP_TOO_MANY_REQUESTS, $headers);
                }),
                Limit::perMinute(20)->by($request->input('token'))->response(function (Request $request, array $headers) {
                    return response()->json([
                        'message' => 'Too many login attempts for this email. Please try again later.',
                    ], Response::HTTP_TOO_MANY_REQUESTS, $headers);
                }),
